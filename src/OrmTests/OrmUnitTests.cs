...
public class OrmUnitTests
{
    ...
    public OrmUnitTests()
    {
        _mapper = new QueryMapper(Environment.GetEnvironmentVariable("TEST_DB_CONNECTION_STRING")!);
    }
    ...
    public async void QueryMapper_Should_ExecuteAndReturnAsync(FormattableString input, string expected)
    {
        ...
        var result = await _mapper.ExecuteAndReturnAsync<string>(input, ctx.Token); 
    ...
    public async void QueryMapper_Should_ExecuteAndReturnWithJoinAsync(FormattableString input, UserArtwork expected)
    {
        var ctx = new CancellationTokenSource();
        
        var result = await _mapper.ExecuteAndReturnAsync<UserArtwork>(input, ctx.Token); 
    ...
    public async void QueryMapper_Should_ExecuteAndReturnListAsync(FormattableString input, List<User> expected)
    {
        var ctx = new CancellationTokenSource();
        
        var result = await _mapper.ExecuteAndReturnListAsync<User>(input, ctx.Token); 
