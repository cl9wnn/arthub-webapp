...
public class UserService(UserRepository userRepository)
{
    ...
    {
        if (await userRepository.GetUserAsyncByLogin(user.Login, cancellationToken) != null)
    ...
    public async Task<Result<JwtTokenModel>> LoginUserAsync(LoginModel model, CancellationToken cancellationToken)
    {
        ...
        var user = await userRepository.GetUserAsyncByLogin(model.Login, cancellationToken);
